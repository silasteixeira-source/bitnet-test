...
def authenticate_gmail():
    SCOPES = ['https://www.googleapis.com/auth/gmail.send']
    # Save token next to executable, not in MEIPASS
    if getattr(sys, 'frozen', False):
...
if __name__ == '__main__':
    root = tk.Tk()
    root.title("Envio de Pleito Standalone")
    root.geometry("900x700")
    ThemeManager.apply_theme_to_all(root)
    app = EnviadorPleitoApp(root, lambda: sys.exit(0))
    # Remove the back button as there is no main menu
    for child in app.frame.winfo_children():
        if isinstance(child, tk.Button) and "VOLTAR" in child.cget("text"):
            child.destroy()
    root.mainloop()
