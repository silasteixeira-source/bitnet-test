...
def authenticate_gmail_readonly():
    ...
    pasta_equipe = os.path.join(base_path, "tokens_equipe")
    os.makedirs(pasta_equipe, exist_ok=True)
    
    token_path_antigo = os.path.join(base_path, "token_leitura.json")
    token_path = os.path.join(pasta_equipe, "token_leitura.json")
    
    # Migrar arquivo antigo para a pasta nova se necessário
    if os.path.exists(token_path_antigo) and not os.path.exists(token_path):
        import shutil
        shutil.move(token_path_antigo, token_path)
...
def get_all_gmail_services():
    services = []
    # 1. Padrão (token_leitura.json)
    try:
        main_service = authenticate_gmail_readonly()
        if main_service:
            perfil = main_service.users().getProfile(userId='me').execute()
            meu_email = perfil.get('emailAddress', '').lower()
            services.append((main_service, meu_email, "Minha Conta"))
    except Exception as e:
        print("Erro na conta principal:", e)
        pass
        
    # 2. Tokens da equipe
    pasta_equipe = "tokens_equipe"
    if os.path.exists(pasta_equipe):
        for f in os.listdir(pasta_equipe):
            if f.endswith('.json') and f != "token_leitura.json":
                caminho = os.path.join(pasta_equipe, f)
                try:
                    creds = Credentials.from_authorized_user_file(caminho, ['https://www.googleapis.com/auth/gmail.readonly'])
                    ...
                    if not any(email == s[1] for s in services):
                        services.append((svc, email, email))
                except Exception as e:
                    print(f"Erro ao carregar token {f}:", e)
    return services
...
class EnviadorPleitoApp:
    ...
    def _enviar_email_bg(self, dados, to_email, cc_email, html_body, assunto):
        ...
        try:
            ...
            try:
                gmail_readonly = authenticate_gmail_readonly()
                inep_str = str(dados.get('inep', '')).strip()
                if inep_str and gmail_readonly:
                    query = f"subject:{inep_str} in:sent"
                    results = gmail_readonly.users().messages().list(userId='me', q=query).execute()
                    messages = results.get('messages', [])
                    if messages:
                        msg_data = gmail_readonly.users().messages().get(userId='me', id=messages[0]['id'], format='metadata', metadataHeaders=['Date', 'Subject']).execute()
                        headers = msg_data['payload']['headers']
                        assunto_duplicado = next((h['value'] for h in headers if h['name'] == 'Subject'), "Desconhecido")
                        data_envio = next((h['value'] for h in headers if h['name'] == 'Date'), "Desconhecida")
                        
                        raise Exception(f"PLEITO JÁ ENVIADO ANTERIORMENTE!\n\nA automação foi BLOQUEADA pois o sistema detectou que já existe um pleito enviado para o INEP {inep_str}.\n\nAssunto original: {assunto_duplicado}\nData de envio: {data_envio}")
            except Exception as check_error:
                if "PLEITO JÁ ENVIADO" in str(check_error):
                    raise check_error # Repassa a trava para interromper o envio
                # Se for erro de timeout ou token de leitura, ignora e segue o envio
                print(f"Aviso: Não foi possível checar duplicidade. {check_error}")
            # --- FIM DA TRAVA DE DUPLICIDADE ---
...
class GerenciadorTokensApp:
    def __init__(self, parent_frame, main_app_callback, gspread_client=None):
        ...
        tk.Label(self.container, text="🔑 GERENCIADOR DE TOKENS", font=("Arial", 16, "bold")).pack(anchor="w", pady=(0, 20))
        
        tk.Label(self.container, text="Tokens para Leitura de E-mails (Robô Leitor):", font=("Arial", 12, "bold")).pack(anchor="w", pady=(10, 5))
        
        frame_botoes = tk.Frame(self.container)
        frame_botoes.pack(fill=tk.X, pady=(0, 20))
        
        tk.Button(frame_botoes, text="Gerar Token de Leitura (Sua Conta)", command=self.gerar_leitura, bg="#0d6efd", fg="white", font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=5)
        tk.Button(frame_botoes, text="Importar Token (Da Equipe)", command=self.importar_token, bg="#198754", fg="white", font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=5)
        
        tk.Label(self.container, text="Token para Envio de E-mails (Plataforma Web):", font=("Arial", 12, "bold")).pack(anchor="w", pady=(20, 5))
        tk.Button(self.container, text="Gerar Token de Envio Web", command=self.gerar_envio, bg="#dc3545", fg="white", font=("Arial", 10, "bold")).pack(anchor="w", padx=5)

    def gerar_envio(self):
        client_secret_file = 'AuthNOC.json'
        if not os.path.exists(client_secret_file):
            messagebox.showerror("Erro", "Arquivo AuthNOC.json não encontrado.")
            return
        ...
        if messagebox.askyesno("Gerar Token Web", "Isto abrirá o navegador para que você faça login com sua conta do Google.\n\nAo finalizar, um arquivo 'meu_token_envio.json' será criado na sua pasta de DOWNLOADS.\n\nDeseja continuar?"):
    ...
    def gerar_leitura(self):
        ...
        def run_flow_leitura():
            try:
                ...
                os.makedirs('tokens_equipe', exist_ok=True)
                token_filename = os.path.join('tokens_equipe', f"token_{email}.json")
                with open(token_filename, 'w') as token:
                    token.write(creds.to_json())
                    
                msg = f"Token de leitura gerado com sucesso!\n\nConta: {email}"
                self.frame.after(0, lambda: messagebox.showinfo("Sucesso", msg))
            except Exception as e:
                self.frame.after(0, lambda: messagebox.showerror("Erro Fatal", f"Ocorreu um erro:\n{e}"))

        if messagebox.askyesno("Gerar Token Leitura", "Isto abrirá o navegador. Faça o login na conta do Google.\nDeseja continuar?"):
            threading.Thread(target=run_flow_leitura, daemon=True).start()

    def importar_token(self):
        arq = filedialog.askopenfilename(filetypes=[("JSON Files", "*.json")], title="Selecione o Token JSON")
        if arq:
            try:
                creds = Credentials.from_authorized_user_file(arq, ['https://www.googleapis.com/auth/gmail.readonly'])
                if creds and creds.valid:
                    pass
                elif creds and creds.expired and creds.refresh_token:
                    creds.refresh(Request())
                ...
                os.makedirs('tokens_equipe', exist_ok=True)
                destino = os.path.join('tokens_equipe', f"token_{email}.json")
                import shutil
                shutil.copy(arq, destino)
                messagebox.showinfo("Sucesso", f"Token importado com sucesso!\nConta vinculada: {email}")
            except Exception as e:
                messagebox.showerror("Erro", f"Token inválido ou ocorreu um erro:\n{e}")
...
class RoboLeitorGmailApp:
    ...
    def acao_buscar(self):
        ...
        try:
            # 1. Carregar todos os serviços do Gmail disponíveis
            services = get_all_gmail_services()
            if not services:
                messagebox.showwarning("Erro", "Nenhuma conta autenticada disponível. Gere ou importe um token de leitura primeiro.")
            ...
            # 2. Buscar em todas as contas
            for gmail_service, email_conta, nome_conta in services:
                try:
                    resultados = gmail_service.users().messages().list(userId='me', q=termo, maxResults=20).execute()
                    msgs_encontradas = resultados.get('messages', [])
                    ...
                    for msg in msgs_encontradas:
                        if msg['id'] not in ids_processados:
                            ...
                            # Adicionar info da conta de origem no payload para usar depois
                            dados_msg['_conta_origem'] = nome_conta
                            dados_msg['_email_origem'] = email_conta
                            
                            mensagens_totais.append(dados_msg)
                except Exception as e:
                    print(f"Erro ao buscar na conta {email_conta}: {e}")
            ...
            # Parse emails
            for dados_msg in mensagens_totais:
                ...
                email_origem = dados_msg.get('_email_origem', '')
                if email_origem and email_origem not in remetente.lower():
                    tem_resposta = True
                elif 'me' not in remetente.lower(): # fallback
                    tem_resposta = True

                corpo_texto = self.extrair_texto_da_mensagem(payload)
                
                origem_str = dados_msg.get('_conta_origem', 'Desconhecida')
                
                corpo_exibicao = f"Encontrado na Caixa: {origem_str}\\n"
                corpo_exibicao += f"De: {remetente}\\nData: {data_formatada}\\n"
                corpo_exibicao += "-" * 50 + "\\n\\n"
                corpo_exibicao += corpo_texto
                
                self.mensagens_cache[dados_msg['id']] = corpo_exibicao
                
                # Inserir na tabela, adicionar a conta de origem no assunto
                assunto_exib = f"[{origem_str}] {assunto}"
                self.tree.insert("", tk.END, iid=dados_msg['id'], values=(data_formatada, remetente, assunto_exib))
...
class MainApp:
    ...
    def show_main_menu(self):
        ...
        buttons = [
            ...
            ("Robô Leitor do Gmail", lambda: self.open(RoboLeitorGmailApp), "#e91e63"),
            ("Gerenciador de Tokens", lambda: self.open(GerenciadorTokensApp), "#6c757d")
        ]
        ...
        tk.Label(footer, text="v6.0 - Otimizado e Unificado", font=("Segoe UI", 8), fg="gray").pack(side="left", pady=10)
    ...
    def gerar_token_web(self):
        ...
        def run_flow():
            client_secret_file = "AuthNoc.json"
            if not os.path.exists(client_secret_file):
                self.root.after(0, lambda: messagebox.showerror("Erro", f"O arquivo '{client_secret_file}' não foi encontrado na pasta atual.\n\nCertifique-se de que a credencial Desktop existe."))
                return
            ...
            try:
                ...
                msg = f"Token gerado com sucesso!\n\nArquivo salvo como: {token_filename}\n\nFaça o upload deste arquivo na página web (Streamlit) para habilitar o envio."
                self.root.after(0, lambda: messagebox.showinfo("Sucesso", msg))
            except Exception as e:
                self.root.after(0, lambda: messagebox.showerror("Erro Fatal", f"Ocorreu um erro ao gerar o token:\n{e}"))

        if messagebox.askyesno("Gerar Token Web", "Isto abrirá o navegador para que você faça login com sua conta do Google.\n\nAo finalizar, um arquivo 'meu_token_envio.json' será criado na sua pasta de DOWNLOADS.\n\nDeseja continuar?"):
            threading.Thread(target=run_flow, daemon=True).start()

    
    def abrir_gerenciador_tokens(self):
        self.nav_frame.pack_forget()
        self.app_atual = GerenciadorTokensApp(self.main_frame, self.show_main_menu)
